...
| Software License | Fully compliant, allows NOSA/GPL license | Fully compliant, allows NOSA/GPL license | Has non-recommended license | Has non open-source license | 
...
Descriptions for each heading are as follows:
   ...
   - HSSI Metadata Compliant?: indicates state of compliance with the new Heliophysics Software Search Interface's (HSSI) metadata requirements. The full metadata standards to be defined in a future PHEP for the HSSI and PyHC connection. For now, the abridged requirements for each level are as follows:
